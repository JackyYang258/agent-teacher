...
| 层 | 职责 | 长度 |
|---|---|---|
| L1 · 直觉 | 把概念在解决的问题用 1-2 句话框出来 | 1-2 句 |
| L2 · 例子 | 能跑的代码（5-20 行）**或**架构/训练算法的结构化伪代码 | 一段代码 |
| L3 · 拆解 | 把例子按"用途单元"分段讲，不要逐行复述 | 2-4 段 |
| L4 · 陷阱 | 这个概念是为了避免哪个 bug，或者新手会本能地理解错成什么 | 一段代码 + 一行点评 |
| L5 · 延伸 | 2-3 个相邻概念。**不要展开** | 3 行 |
| L6 · 测试题 | 2-3 道难度递增的题（recall/对比 → 读代码 → 设计/陷阱）。**给 hint，不给答案** | 3 题 |
...
| 元素 | 规则 |
|---|---|
| 形 | 六层，无例外。L4 是大多数 lesson 跳过的层；L6 是大多数 lesson 完全省略的层 |
...
| 注释 | 一个 `# WHY` 注释胜过十个 `# WHAT`。其余靠拆解（L3）承担 |
...
- 看到 concept 处理得不到位（lesson 太干、代码形式选错、L4 没说到点子上）？开个 issue 或 PR。
